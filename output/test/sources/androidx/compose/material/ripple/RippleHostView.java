...
public final class RippleHostView extends View {
    ...
    public final void m1283addRippleKOepWvA(PressInteraction.Press interaction, boolean bounded, long size, int radius, long color, float alpha, Function0<Unit> onInvalidateRipple) {
        ...
        m1284updateRipplePropertiesbiQXAtU(size, radius, color, alpha);
        if (bounded) {
            ripple.setHotspot(Offset.m2710getXimpl(interaction.getPressPosition()), Offset.m2711getYimpl(interaction.getPressPosition()));
        } else {
    ...
    public final void m1284updateRipplePropertiesbiQXAtU(long size, int radius, long color, float alpha) {
        UnprojectedRipple ripple = this.ripple;
        if (ripple == null) {
            return;
        }
        ripple.trySetRadius(radius);
        ripple.m1291setColorDxMtmZc(color, alpha);
        Rect newBounds = new Rect(0, 0, MathKt.roundToInt(Size.m2779getWidthimpl(size)), MathKt.roundToInt(Size.m2776getHeightimpl(size)));
