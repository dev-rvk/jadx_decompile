...
public final class PlatformMagnifierFactoryApi29Impl implements PlatformMagnifierFactory {
    ...
    public PlatformMagnifierImpl create(MagnifierStyle style, View view, Density density, float initialZoom) {
        ...
        if (!Intrinsics.areEqual(style, MagnifierStyle.INSTANCE.getTextDefault())) {
            long size = density.mo330toSizeXkaWNTQ(style.getSize());
            float cornerRadius = density.mo329toPx0680j_4(style.getCornerRadius());
            float elevation = density.mo329toPx0680j_4(style.getElevation());
            Magnifier.Builder $this$create_u24lambda_u241_u24lambda_u240 = new Magnifier.Builder(view);
            if (size != Size.INSTANCE.m2787getUnspecifiedNHjbRc()) {
                $this$create_u24lambda_u241_u24lambda_u240.setSize(MathKt.roundToInt(Size.m2779getWidthimpl(size)), MathKt.roundToInt(Size.m2776getHeightimpl(size)));
            }
    ...
    public static final class PlatformMagnifierImpl extends PlatformMagnifierFactoryApi28Impl.PlatformMagnifierImpl {
        ...
        public void mo246updateWko1d7g(long sourceCenter, long magnifierCenter, float zoom) {
            if (!Float.isNaN(zoom)) {
                getMagnifier().setZoom(zoom);
            }
            if (OffsetKt.m2729isSpecifiedk4lQ0M(magnifierCenter)) {
                getMagnifier().show(Offset.m2710getXimpl(sourceCenter), Offset.m2711getYimpl(sourceCenter), Offset.m2710getXimpl(magnifierCenter), Offset.m2711getYimpl(magnifierCenter));
            } else {
                getMagnifier().show(Offset.m2710getXimpl(sourceCenter), Offset.m2711getYimpl(sourceCenter));
            }
        }
    }
}
