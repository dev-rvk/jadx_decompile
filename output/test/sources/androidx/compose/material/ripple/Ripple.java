...
public abstract class Ripple implements Indication {
    ...
    public abstract RippleIndicationInstance mo1277rememberUpdatedRippleInstance942rkJo(InteractionSource interactionSource, boolean z, float f, State<Color> state, State<RippleAlpha> state2, Composer composer, int i);
    ...
    public final IndicationInstance rememberUpdatedInstance(InteractionSource interactionSource, Composer $composer, int $changed) {
        long mo1121defaultColorWaAFU9c;
        ...
        long $this$isSpecified$iv = this.color.getValue().m2959unboximpl();
        if ($this$isSpecified$iv != Color.INSTANCE.m2985getUnspecified0d7_KjU()) {
            mo1121defaultColorWaAFU9c = this.color.getValue().m2959unboximpl();
        } else {
            mo1121defaultColorWaAFU9c = theme.mo1121defaultColorWaAFU9c($composer, 0);
        }
        $composer.endReplaceableGroup();
        State color = SnapshotStateKt.rememberUpdatedState(Color.m2939boximpl(mo1121defaultColorWaAFU9c), $composer, 0);
        State rippleAlpha = SnapshotStateKt.rememberUpdatedState(theme.rippleAlpha($composer, 0), $composer, 0);
        RippleIndicationInstance instance = mo1277rememberUpdatedRippleInstance942rkJo(interactionSource, this.bounded, this.radius, color, rippleAlpha, $composer, ($changed & 14) | (($changed << 12) & 458752));
    ...
    public boolean equals(Object other) {
        ...
        return (other instanceof Ripple) && this.bounded == ((Ripple) other).bounded && Dp.m5223equalsimpl0(this.radius, ((Ripple) other).radius) && Intrinsics.areEqual(this.color, ((Ripple) other).color);
    }

    public int hashCode() {
        int result = Boolean.hashCode(this.bounded);
        return (((result * 31) + Dp.m5224hashCodeimpl(this.radius)) * 31) + this.color.hashCode();
    }
}
